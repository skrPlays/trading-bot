...
def EN_STRATEGY(df):
    ...
    # Bollinger combo
    buybollinger = (close[lnx-2] < bb_hband[lnx-2]) and (close[lnx-1] > bb_hband[lnx-1]) and (close[lnx] < bb_hband[lnx])
    sellbollinger = (close[lnx-2] > bb_lband[lnx-2]) and (close[lnx-1] < bb_lband[lnx-1]) and (close[lnx] > bb_lband[lnx])
    ...
    buyflag = (adx_buy_diratio > 2.5) and (close[lnx-2] < bb_hband[lnx-2]) and (close[lnx-1] > bb_hband[lnx-1]) and (close[lnx] < bb_hband[lnx])
    sellflag = (adx_sell_diratio > 2.5) and (close[lnx-2] > bb_lband[lnx-2]) and (close[lnx-1] < bb_lband[lnx-1]) and (close[lnx] > bb_lband[lnx])
...
def readlosslimitflag(df, close, lnx, entryprice, ordertype):
    ...
    flag = DecisionFunctions.position_decision([maxprofitprice], [close[lnx]], [entryprice], ["WHATEVS"], [ordertype], kiteconnect.KiteConnect)
