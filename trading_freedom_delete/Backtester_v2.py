    ...
    for day in {15, 16, 17, 18, 19}:
        ...
        for i in range(5, 20):  # Todo: no position in first few and last few periods
            ...
            if position_flag:
                ...
                position["tradingsymbol"], position["action"], position["price"], position_flag = ENTRY(
                    offset, position_flag
                )

        if position_flag:
            initialfund = initialfund * (
                1 + settlement(offset, position["tradingsymbol"], position["action"], position["price"])
            )
        print(initialfund)
...
def ENTRY(offset, position_flag):
    ...
    for scrip in nifty_list:
        df_original = pandas.read_csv(
            "C:\wamp\www\TradeFreedom\BackTesting\HistoricalData\\" + scrip["tradingsymbol"] + ".csv"
        )
...
def EXIT(offset, positionflag, position):
    ...
    df_original = pandas.read_csv(
        "C:\wamp\www\TradeFreedom\BackTesting\HistoricalData\\" + position["tradingsymbol"] + ".csv"
    )
    df_original.index = df_original["date"]
    df_original.index.name = "timestamp"
    df = df_original.iloc[: len(df_original) + offset + 1]
