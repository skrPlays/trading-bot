    ...
    for net in kite.positions()["net"]:
        ...
        if net["quantity"] != 0:
            if net["quantity"] / abs(net["quantity"]) == 1:
                ...
                max_profit_price.append(max_profit_json[str(net["instrument_token"])]) if str(
                    net["instrument_token"]
                ) in max_profit_json else max_profit_price.append(net["buy_price"])
                ...
                max_profit_price.append(max_profit_json[str(net["instrument_token"])]) if str(
                    net["instrument_token"]
                ) in max_profit_json else max_profit_price.append(net["sell_price"])
                last_price.append(ltpresponse[str(net["instrument_token"])])
