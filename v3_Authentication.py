...
def authenticate():
    ...
    headers = {
        ...
        "X-Kite-Userid": myuserID,
        "X-Kite-Version": "2.4.0",
        "Cookie": cookies,
        "Referer": login.url,
    }
    data = requests.post(
        "https://kite.zerodha.com/api/login", {"user_id": myuserID, "password": myPW}, headers=headers
    )
    data = requests.post(
        "https://kite.zerodha.com/api/twofa",
        {"user_id": myuserID, "request_id": data.json()["data"]["request_id"], "twofa_value": my2fa},
        headers=headers,
    )
