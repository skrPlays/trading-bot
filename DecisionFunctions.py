...
def position_decision(maxProfitPrice, ltp, entryPrice, scripName, orderType, kite):
    ...
    for i in range(len(maxProfitPrice)):
        if  not (scripName[i] == "SBIN" or scripName[i] == "IDEA" or scripName[i] == "TATAPOWER"):
            ...
            print("\033[1m" + scripName[i] + " " + str(datetime.datetime.now()),
                  "\033[0mPermissible Pullback: "+str(round(permissiblePullback(max_change_per*orderType[i]),2))+"%",
                  colored("Highest Swing: "+str(round(max_change_per,2))+"%","blue" if orderType[i] > 0 else "red"),
                  colored("Current: "+str(round(current_change_per,2))+"%","blue" if current_change_per > 0 else "red"),
                  colored("Distance from Highest: "+str(round((max_change_per-current_change_per)*orderType[i],2))+"%","yellow"),end=' ')

            exitflag = exitFlag((max_change_per-current_change_per)*orderType[i],permissiblePullback(max_change_per*orderType[i]))
            if (exitflag):
                # placeorder(kite)
...
def placeorder(kite):
    try:
        print(kite.place_order(transaction_type="BUY",quantity=1,exchange="NSE",tradingsymbol="COALINDIA",variety="regular",product="MIS",order_type="MARKET"))
